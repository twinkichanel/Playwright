...
exports.kUnableToAdoptErrorMessage = exports.InjectedScriptPollHandler = exports.ElementHandle = exports.FrameExecutionContext = exports.NonRecoverableDOMError = void 0;
...
class ElementHandle extends js.JSHandle {
  ...
  async _performPointerAction(progress, actionName, waitForEnabled, action, forceScrollOptions, options) {
    ...
    if (!process.env.PLAYWRIGHT_LAYOUT_SHIFT_CHECK) return this._finishPointerAction(progress, actionName, point, options, action);else return this._finishPointerActionDetectLayoutShift(progress, actionName, point, options, action);
  }

  async _finishPointerAction(progress, actionName, point, options, action) {
    if (!options.force) {
  ...
  async _finishPointerActionDetectLayoutShift(progress, actionName, point, options, action) {
    await progress.beforeInputAction(this);
